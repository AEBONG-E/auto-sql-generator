...
@Slf4j
@Component
public class ExcelSchemaExtractorImpl implements ExcelSchemaExtractor {
    ...
    @Override
    public List<TableSchema> extract(ExtractedSchemaFile file) throws Exception {
        try (Workbook wb = WorkbookFactory.create(new ByteArrayInputStream(file.content()))) {
    ...
    private List<TableSchema> buildSchemas(Sheet sheet, Map<String, Integer> idx) {
        ...
        log.debug("Extracted {} tables from file", result.size());
        return result;
    }
